...
export async function loadConfig() {
  return {
    ...
    openaiModel: process.env.BLOG_OPENAI_MODEL ?? "gpt-6-luna",
    openaiBaseUrl: process.env.OPENAI_BASE_URL ?? "https://api.openai.com/v1",
    runIntervalMinutes: integer("BLOG_RUN_INTERVAL_MINUTES", 360, 60),
    maxRunsPerDay: integer("BLOG_AI_MAX_RUNS_PER_DAY", 3, 1),
    maxInputChars: integer("BLOG_AI_MAX_INPUT_CHARS", 28000, 4000),
    maxOutputTokens: integer("BLOG_AI_MAX_OUTPUT_TOKENS", 16000, 500),
    dailyTokenBudget: integer("BLOG_AI_DAILY_TOKEN_BUDGET", 80000, 1000),
    monthlyCostBudgetUsd: number("BLOG_AI_MONTHLY_COST_USD", 5, 0.1),
    inputPricePerMillionUsd: number("BLOG_AI_INPUT_PRICE_PER_MILLION_USD", 0.1, 0),
    outputPricePerMillionUsd: number("BLOG_AI_OUTPUT_PRICE_PER_MILLION_USD", 0.5, 0),
    ...
    telegramEnabled: boolean("BLOG_TELEGRAM_ENABLED", false),
    telegramBotToken: await readSecret("TELEGRAM_BOT_TOKEN", "TELEGRAM_BOT_TOKEN_FILE"),
    telegramWebhookSecret: await readSecret("TELEGRAM_WEBHOOK_SECRET", "TELEGRAM_WEBHOOK_SECRET_FILE"),
    reviewSigningSecret: await readSecret("BLOG_REVIEW_SIGNING_SECRET", "BLOG_REVIEW_SIGNING_SECRET_FILE"),
    telegramChatId: (process.env.BLOG_TELEGRAM_CHAT_ID ?? "").trim(),
    telegramUserId: (process.env.BLOG_TELEGRAM_USER_ID ?? "").trim(),
    reviewTtlHours: integer("BLOG_REVIEW_TTL_HOURS", 72, 1),
